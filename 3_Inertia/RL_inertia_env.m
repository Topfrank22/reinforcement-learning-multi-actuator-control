classdef RL_inertia_env < rl.env.MATLABEnvironment
    ...
    properties
        ...
        % ===== MOTOR INERTIA MODEL =====
        m_boxes = zeros(10,1);       % Massa di ogni pacco [kg]
        vx_boxes = zeros(10,1);      % Velocità effettiva X di ogni pacco [m/s]
        % Parametri del modello motore
        tau_0 = 0.02;                % Costante di tempo base del motore [s] (a vuoto)
        tau_k = 0.015;               % Fattore inerziale: tau = tau_0 + tau_k * m_box [s/kg]
        min_mass = 1.0;              % Massa minima pacco [kg]
        max_mass = 10.0;             % Massa massima pacco [kg]
    end
    ...
    methods              
        ...
        function [Observation,Reward,IsDone,Info] = step(this,Action)
            ...
            if mod(round(this.time,2),0.75) == 0 && this.n_boxes_tot<this.curriculum_limit && this.time>0.25
                ...
                % Assign random masses to newly spawned boxes
                for mm = (this.n_boxes_tot - gen_n_boxes + 1):this.n_boxes_tot
                    this.m_boxes(mm) = this.min_mass + (this.max_mass - this.min_mass) * rand(1);
                end
            ...
            for ii=1:this.n_boxes_tot
                ...
                % packages kinematics — MOTOR INERTIA MODEL
                % Il motore della cella ha un ritardo dipendente dal carico:
                %   tau_i = tau_0 + tau_k * m_box
                % La velocità effettiva del nastro (e del pacco solidale)
                % segue un filtro passa-basso del primo ordine:
                %   V_eff(t+dt) = V_eff(t) + (dt/tau_i) * (V_target - V_eff(t))

                tau_i = this.tau_0 + this.tau_k * this.m_boxes(ii);
                alpha = this.dt / tau_i;  % fattore di smorzamento [0,1]

                if this.i_box(ii) < 6
                    % Velocità target comandata dall'agente
                    vx_target = v_AMS(this.i_box(ii),this.j_box(ii)) * sin(rotation_AMS(this.i_box(ii),this.j_box(ii)));
                    vy_target = v_AMS(this.i_box(ii),this.j_box(ii)) * cos(rotation_AMS(this.i_box(ii),this.j_box(ii)));
                    
                    % Filtro passa-basso (inerzia motore sotto carico)
                    this.vx_boxes(ii) = this.vx_boxes(ii) + alpha * (vx_target - this.vx_boxes(ii));
                    this.vy_boxes(ii) = this.vy_boxes(ii) + alpha * (vy_target - this.vy_boxes(ii));
                    
                    % Aggiornamento posizione con velocità effettiva
                    this.x_boxes(ii) = this.x_boxes(ii) + this.vx_boxes(ii) * this.dt;
                    this.y_boxes(ii) = this.y_boxes(ii) + this.vy_boxes(ii) * this.dt;
                else
                    % Sul treadmill: velocità costante istantanea (il treadmill non è limitato dai motori delle celle)
                    this.vx_boxes(ii) = 0;
            ...
            for ii = 1:this.max_gen_boxes
                ...
                if ii <= this.n_boxes_tot
                    ...
                    Observation(base+7)  = this.m_boxes(ii) / this.max_mass;                   % Massa norm [0,1]
        ...
        function InitialObservation = reset(this)
            ...
            for ii = 1:this.max_gen_boxes
                this.m_boxes(ii) = this.min_mass + (this.max_mass - this.min_mass) * rand(1);
            end
            ...
            for ii = 1:this.max_gen_boxes
                ...
                if ii <= this.n_boxes_tot
                    ...
                    InitialObservation(base+7)  = this.m_boxes(ii) / this.max_mass;
            ...
            for ii = 1:this.n_boxes_tot
                for jj = ii+1:this.n_boxes_tot
                    if this.pack_exited(ii)==0 && this.pack_exited(jj)==0
                        ...
                        % Soglia dinamica basata sulla massa del pacco posteriore (che frena con ritardo)
                        m_trail = this.m_boxes(ii);
                        if this.y_boxes(jj) < this.y_boxes(ii)
                            m_trail = this.m_boxes(jj);
                        end
                        d_gap_cap_y = 0.15 + 0.015 * m_trail; % da 0.165m (1kg) a 0.30m (10kg)
                        
                        Reward = Reward + min(gap_y, d_gap_cap_y) * 2.0;
                    end
                end
            end

            % 6. VELOCITY GRADIENT REWARD (Early Separation Incentive)
            %    Se due pacchi sono troppo vicini in Y, premia la differenza di velocità
            ...
            for ii = 1:this.n_boxes_tot
                for jj = ii+1:this.n_boxes_tot
                    if this.pack_exited(ii)==0 && this.pack_exited(jj)==0
                        ...
                        % Soglia dinamica basata sulla massa del pacco posteriore
                        m_trail = this.m_boxes(ii);
                        if this.y_boxes(jj) < this.y_boxes(ii)
                            m_trail = this.m_boxes(jj);
                        end
                        threshold_y = 0.15 + 0.015 * m_trail;
